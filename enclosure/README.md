...
v0.3.1 cuts **12 radial channels** (2.0 mm wide × 0.8 mm deep) running from r≈13 to r≈42. They
...
felt.) The channel walls are **tapered ~40°** from vertical (`TaperAngle` on the pocket) — wide
at the glass face, narrowing to a ~0.8 mm floor. Since the Top prints face-down, the channel is
a recess in the bed-side surface; a square `]` profile would leave a flat, down-facing floor
that bridges and prints rough, whereas the taper makes the walls self-supporting overhangs and
shrinks that bridge. (A self-supporting *full* V — no flat floor — needs width ≤ 2× depth, i.e.
~1.6 mm at this 0.8 mm depth; 2.0 mm/40° is the chosen middle ground between width and a small
floor.) It also drains better — no square corners to trap water. Radial (not a grid or field)
also prints well: the rim between channels stays flat, and the flat r42–45 band gives the
face-down print a clean, continuous first-layer perimeter.
